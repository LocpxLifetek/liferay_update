		...
		<div class="listDirectOperationRight">
			<c:forEach items="${listBlogs}" var="blog">
				...
				<div class="listDirectOperationGrid">
					<div class="ldo">
						<a href="${url}/chitiettintuc?uuid=${blog.uuidBlogsEntry}"> <img
							src="${blog.src}" class="soup-image"
							style="width: 30% !important">
						</a> <a href="${url}/chitiettintuc?uuid=${blog.uuidBlogsEntry}">${blog.titleBlogsEntry}</a>
...
<div id="wrapper">
	<ul id="pagination">
		<c:if test="${currentPage != 1}">
			<li><a href="${url}/tintuc?uuid=${uuid}&page=${currentPage - 1}">«</a></li>
		</c:if>
			...
			<c:choose>
				...
				<c:otherwise>
					<c:if test="${(currentPage-3) lt i and i lt (currentPage + 3)}">
						<li><a href="${url}/tintuc?uuid=${uuid}&page=${i}">${i}</a></li>
					</c:if>
				</c:otherwise>
			</c:choose>
		</c:forEach>
		...
		<c:if test="${currentPage lt totalPage}">
			<li><a href="${url}/tintuc?uuid=${uuid}&page=${currentPage + 1}">»</a>
			</li>
		</c:if>
