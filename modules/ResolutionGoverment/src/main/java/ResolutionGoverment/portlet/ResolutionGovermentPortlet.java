...
@Component(
	...
	property = {
		...
		"javax.portlet.display-name=ResolutionGoverment",
...
public class ResolutionGovermentPortlet extends MVCPortlet {
	...
	private List<JournalArticleDto> findAllJournalArticleAndDontId(Long idJournalArticle) throws SQLException{
		try {
			...
			statement=con.prepareStatement("SELECT\r\n" + 
					...
					"WHERE\r\n" + 
					"        ja.folderid = '189126'\r\n" + 
					"    AND ja.ddmstructurekey='189235' and ja.id_!=?\r\n" + 
					"GROUP BY\r\n" + 
					"    ja.resourceprimkey order by max(ja.id_) desc offset 0 rows fetch next 8 rows only");
	...
	private JournalArticleLocazationDto findAllJornalArticleByArticlePK(long articlePk,long id) {
		try {
			con = DataAccess.getConnection();
			statement=con.prepareStatement(" select ja.title from journalarticlelocalization ja where ja.articlePk=?");
	...
	public void doView(RenderRequest renderRequest, RenderResponse renderResponse)
		...
		try {
			...
			JSONObject json=parseContent(journalArticleObject.getContent(),journalArticleObject.getDefaultLanguageId());
			...
			try {
				...
				for (JournalArticleDto journalArticleDto : listJournalArticleDtos) {
					JournalArticle journalArticle=JournalArticleLocalServiceUtil.getArticle(journalArticleDto.getId());
					JournalArticleLocazationDto journalArticleLocazationDto=findAllJornalArticleByArticlePK(journalArticle.getId(),journalArticle.getId());
